export const PLATFORM_SPECS = {
  instagram: {
    label: "인스타그램 릴스 캡션",
    rules: "첫 줄 40자 안에 후킹. 전체 300~600자. 줄바꿈으로 호흡 나누기. 해시태그 8~12개는 hashtags 로 분리. 링크는 본문에 못 넣으니 '프로필 링크' 로 유도.",
  },
  threads: {
    label: "스레드",
    rules: "500자 이내. 대화체로 질문 던지며 시작. 해시태그는 1~2개만.",
  },
  x: {
    label: "X(트위터)",
    rules: "한글 220자 이내로 완결. 줄바꿈 2회 이내. 해시태그 2개 이내. 링크 자리는 본문 끝에 비워두기.",
  },
  youtube: {
    label: "유튜브 쇼츠",
...
export function systemPrompt(brand) {
  const banned = (brand.banned ?? []).join(", ");
  return [
    ...
    `- 이름: ${brand.name}`,
    `- 톤앤매너: ${brand.voice}`,
    brand.cta ? `- 기본 CTA: ${brand.cta}` : "",
    "",
    "# 반드시 지킬 것",
    "1. 원문(블로그/화면)에 없는 사실, 숫자, 할인율을 지어내지 않습니다. 근거가 없으면 그 표현을 뺍니다.",
    banned ? `2. 다음 표현은 절대 쓰지 않습니다: ${banned}` : "2. 근거 없는 최상급 표현을 쓰지 않습니다.",
...
export function userPrompt({ article, screenText, platforms, variants, language }) {
  const specs = platforms
    .map((p) => `- ${p} (${PLATFORM_SPECS[p]?.label ?? p}): ${PLATFORM_SPECS[p]?.rules ?? "간결하게"}`)
    .join("\n");
  ...
  return [
    ...
    article
      ...
      : "(블로그 글 없음)",
    "",
    screenText ? `# 함께 붙일 영상에 담긴 화면 흐름\n${screenText}` : "",
    "",
    "# 만들어야 할 것",
    `언어: ${language === "ko" ? "한국어" : language}`,
    `플랫폼별로 서로 다른 시안 ${variants}개씩:`,
    specs,
    "",
