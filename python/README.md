...
| 화면 크기 | **브라우저 창** 크기입니다. 결과물은 항상 1080×1920 세로 (아래 참고) |
...
### 가로가 긴 화면을 세로 영상에 담을 때

결과물은 항상 **1080×1920 세로**입니다. 창의 "화면 크기" 는 결과물 크기가 아니라
**브라우저 창** 크기입니다. 그래서 데스크톱(1280×800)처럼 가로가 긴 화면으로 녹화하면
세로 규격과 비율이 크게 다릅니다.

꽉 채우려고 잘라내면 **가로의 35% 만 남습니다.** 그래서 기본은 잘라내지 않고
전체를 넣은 뒤, 남는 위아래를 같은 화면을 흐리게 깔아 채웁니다.
...
| 값 | 동작 |
|---|---|
| `auto` (기본) | 많이 잘릴 것 같으면 여백을 두고, 비율이 맞으면 꽉 채웁니다 |
| `fill` | 항상 꽉 채우고 넘치는 부분은 잘라냅니다 |
| `contain` | 항상 전체를 넣고 여백을 둡니다 |

| 화면 크기 | 실제 프레임 | `auto` 판단 |
|---|---|---|
| 모바일 540×960 | 1080×1920 | 꽉 채우기 (잘림 없음) |
| 태블릿 768×1024 | 1536×2048 | 여백 두기 (채우면 25% 잘림) |
| 데스크톱 1280×800 | 1280×800 | 여백 두기 (채우면 65% 잘림) |

**세로로 꽉 찬 영상을 원하시면 모바일 크기로 녹화하세요.** 그게 숏츠 규격과 정확히 맞습니다.
...
기본 녹화는 **동의·확인·닫기 같은 팝업 버튼만 자동으로 찾아서** 누르고, 나머지는 스크롤만 합니다.
"이 버튼을, 이 시각에" 처럼 정확히 지정하고 싶을 때 이 칸을 씁니다.

| 칸 | 뜻 |
|---|---|
| **팝업 자동 닫기** | 끄면 동의·닫기 버튼을 자동으로 찾지 않습니다. 그 버튼도 좌표로 직접 누르고 싶을 때 끄세요. |
| **아래 좌표를 직접 클릭** | 목록에 적은 좌표를 순서대로 누릅니다. 꺼두면 예전과 똑같이 스크롤만 합니다. |
| **시각(초)** | 녹화가 시작된 뒤 몇 초에 누를지. **페이지 여는 시간이 포함**됩니다. |
| **X · Y** | 브라우저 화면 기준 좌표. 왼쪽 위가 (0, 0) 입니다. |
| **스크롤** | 그 좌표를 잡았을 때 페이지를 몇 px 내린 상태였는지. 누르기 전에 이 위치로 맞춥니다. |

**좌표는 '좌표 찍기' 로 넣는 것이 가장 정확합니다.**
누르면 녹화와 **똑같은 창 크기**로 브라우저가 열립니다. 누를 자리를 차례로 클릭한 뒤
창에서 **찍기 완료** 를 누르면 좌표와 스크롤 위치가 그대로 목록에 들어옵니다.
찍는 동안에는 링크·버튼이 동작하지 않아 페이지가 튀지 않습니다.

> **왜 직접 적으면 틀리기 쉬운가** — 브라우저는 **CSS 픽셀**로 클릭하는데, 완성된 영상은
> 그 `deviceScaleFactor` 배 크기입니다. 모바일(540×960)로 찍으면 영상은 1080×1920 이라,
> **영상에서 잰 좌표를 그대로 넣으면 정확히 2배 어긋납니다.**
> 직접 적을 땐 `직접 추가` 창에서 **좌표 기준을 '영상 픽셀'** 로 고르세요 — 알아서 환산합니다.
> 화면 밖 좌표는 브라우저를 띄우기 **전에** 걸러내고, 얼마로 고쳐야 하는지 알려줍니다.

녹화 중에는 클릭할 때마다 **그 자리에 무엇이 있었는지** 진행 기록에 남습니다.
좌표가 맞았는지는 이 줄로 확인하세요.

```
클릭 6.0초 · (270, 100) → <div#b6> 블록 6
```
...
│   ├── clicks.py          지정한 클릭 좌표 검사·환산 (배율·화면 밖·스크롤)
│   ├── pick.py            브라우저에서 좌표를 직접 찍어 오기
