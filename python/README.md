...
| 첫 화면 문구 | 영상 첫 장면에 뜨는 훅. `**강조**` 는 포인트 색으로 나옵니다. 비워도 됩니다 |
| 마지막 화면 문구 | 끝 장면의 큰 문구(CTA). 비우면 설정의 `brand.cta` 를 쓰고, 그것도 비면 안 나옵니다 |
...
### 첫 화면 · 마지막 화면 문구 넣고 빼기

영상 앞뒤에 붙는 카드의 글자는 전부 켜고 끌 수 있습니다. **비우면 그 줄이 아예 안 나옵니다.**

| 위치 | 무엇 | 어디서 정하나 | 기본값 |
|---|---|---|---|
| 첫 화면 위 | 작은 배지 | `brand.badge` | 비어 있음 (안 나옴) |
| 첫 화면 가운데 | 큰 훅 문구 | 창의 **첫 화면 문구** | 비어 있음 |
| 마지막 화면 가운데 | 큰 CTA 문구 | 창의 **마지막 화면 문구** → 없으면 `brand.cta` | `brand.cta` |
| 마지막 화면 아래 | 작은 브랜드 표기 | `brand.signature` | 비어 있음 (안 나옴) |

마지막 화면의 CTA 를 비우면 **아래 화살표까지 같이 빠집니다.**

```yaml
brand:
  cta: "프로필 링크에서 쿠폰 받기"   # 마지막 화면 큰 문구
  badge: ""                          # 첫 화면 위 작은 배지
  signature: ""                      # 마지막 화면 아래 작은 문구
```

### 카드 자체를 빼기

문구가 아니라 **카드 구간을 통째로** 없애려면 초를 `0` 으로 둡니다.
기본값은 아웃트로 없음 — 본편이 끝나면 영상도 바로 끝납니다.

```yaml
render:
  shorts:
    introSeconds: 1.6   # 0 이면 훅 카드 없이 본편부터 시작
    outroSeconds: 0     # 0 이면 마지막 카드 없이 본편에서 바로 끝 (기본값)
```

| 설정 | 영상 구성 | 예시 길이 |
|---|---|---|
| intro 1.6 / outro 2.0 | 훅 카드 → 본편 → 마지막 카드 | 16.6초 |
| intro 1.6 / outro 0 (기본) | 훅 카드 → 본편 | 14.6초 |
| intro 0 / outro 0 | 본편만 | 13.0초 |

둘 다 끄면 표지 이미지(`cover.png`)는 본편 첫 장면에서 뽑습니다.
